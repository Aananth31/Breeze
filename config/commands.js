...
var commands = exports.commands = {
	...
	restarthelp: function (target, room, user) {
		...
		if (!this.canBroadcast()) return;
		this.sendReplyBox(
			"Cafe is restarting. Things to know:<br />" +
			"- We wait a few minutes before restarting so people can finish up their battles.<br />" +
			"- The restart itself will take around (insert unknown number here) seconds.<br />" +
			"- Your ladder ranking and teams will not change.<br />" +
			"- We are restarting to update Cafe to a newer and better version. xD"
		);
	},
	...
	games: function(target,room,user) {
	 	if (!this.canBroadcast()) return;
	 	this.sendReplyBox('<font size=5><font color=red><center>Which Game should we Play Now?</center><center><button name="send" value="**We should play GAME OF REVOLUTION.**" target="_blank">Game of Revolution. (Winner Gets Promoted, Dead Gets Game Locked.)</button> <button name="send" value="**We should START A TOURNAMENT.**" target="_blank">Tournament. (Winner Gets Bucks.)</button> <button name="send" value="/join games" target="_blank">Mafia. (Winner Gets Points.)</button> <button name="send" value="/join games" target="_blank">Scavengers. (Winner Gets Points.)</button> <button name="send" value="/join games" target="_blank">Hangman. (Winner Gets Points.)</button> <button name="send" value="/join games" target="_blank">Trivia. (Winner Gets Points.)</button></center>')
	},
