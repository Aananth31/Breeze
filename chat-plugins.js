		...
		commands: {
			...
			lynch: function(target, room, user) {
				...
				if (totals[targetUser] >= (Math.floor(Object.keys(plugins.mafia.participants).length / 2) +1)) {
					...
					if (targetUser === 'no lynch') {
						...
						if (Rooms.rooms.lobby) Rooms.rooms.lobby.add('|raw|<div class="broadcast-blue"><strong>' + this.targetUsername + ' was lynched and was a ' + plugins.mafia.participants[targetUserid] + '!');
			...
			nightaction: function(target, room, user) {
				...
				if (winner.length === 1) {
					...
					if (Rooms.rooms.lobby) Rooms.rooms.lobby.add('|raw|<div class="broadcast-blue"><strong>It is now day! The remaining players are: ' + Object.keys(plugins.mafia.participants).join(' ') + '</strong></div>');
				}
